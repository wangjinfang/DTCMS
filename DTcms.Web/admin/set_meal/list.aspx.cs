...
namespace DTcms.Web.admin.set_meal
{
    public partial class list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                TreeBind(); //绑定类别
                RptBind(" 1=1 " , "sort_id asc,category_id desc,add_time desc");
            }
        }
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            if (category_id > 0)
            {
                _strWhere += " and category_id in(select id from td_meal_category where class_list like '%," + category_id + ",%')";
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("meal_list_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("meal_list_page_size", _pagesize.ToString(), 14400);
                }
            }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(DTEnums.ActionEnum.Delete.ToString(), "删除套餐" + sucCount + "条，失败" + errorCount + "条"); //记录日志
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(DTEnums.ActionEnum.Edit.ToString(), "保存套餐频道内容排序"); //记录日志
            JscriptMsg("保存排序成功啦！",Utils.CombUrlTxt("list.aspx", "category_id={0}",this.category_id.ToString()), "Success");
        }
