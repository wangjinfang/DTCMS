...
namespace DTcms.Web.admin.settings
{
    public partial class url_rewrite_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("site_url_rewrite", DTEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(); //绑定频道
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == DTEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("site_url_rewrite", DTEnums.ActionEnum.Edit.ToString()); //检查权限
            ...
            {
                ChkAdminLevel("site_url_rewrite", DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
