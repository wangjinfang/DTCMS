...
namespace DTcms.Web.admin.property
{
    public partial class list : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("property_list_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("property_list_page_size", _pagesize.ToString(), 14400);
                }
            }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(DTEnums.ActionEnum.Delete.ToString(), "删除商品属性" + sucCount + "条，失败" + errorCount + "条"); //记录日志
