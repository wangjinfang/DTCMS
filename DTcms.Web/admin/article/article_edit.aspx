...
<script type="text/javascript" src="../js/layout.js"></script>
    <script src="../js/nan.js" type="text/javascript"></script>
...
<script type="text/javascript">
    ...
    function add_unit() {
        if ($("#txt_unit_title").val() == "") {
            $.dialog.alert('请输入单位名称！');
            return ;
        }

        if ($("#txt_unit_quntity").val() == "") {
            $.dialog.alert('请输入数量！');
            return ;
        }
        ...
        if (!reg.test($("#txt_unit_quntity").val())) {
            $.dialog.alert('您输入的格式不正确！');
            return ;
        }


        if ($("#txt_unit_rate").val() == "") {
            $.dialog.alert('请输入比率！');
            return ;
        }
        ...
        if (!reg1.test($("#txt_unit_rate").val())) {
            $.dialog.alert('您输入的格式不正确！');
            return ;
        }

        var val = $("#txt_unit_title").val() + "_" + $("#txt_unit_quntity").val() + "_" + $("#txt_unit_rate").val() + "_" + $("#txt_unit_content").val();

        var str = "<tr>";
        str += "<td style='padding-right: 20px;'>" + $("#txt_unit_title").val() + "<input type=\"checkbox\" name=\"ck_unit\" value=\"" + val + "\" checked=\"checked\" style=\"display:none;\">" + "</td>";
        str += "<td style='padding-right: 20px;'>" + $("#txt_unit_quntity").val() + "</td>";
        str += "<td style='padding-right: 20px;'>" + $("#txt_unit_rate").val() + "</td>";
        str += "<td style='padding-right: 20px;'>" + $("#txt_unit_content").val() + "</td>";
        str += "<td style='padding-right: 20px;'><a href=\"javascript:;\" onclick=\"del_unit(this)\">删除</a></td>";
        str += "</tr>";
        $("#tb_unit_value").append(str);

        $("#txt_unit_title").val("");
        $("#txt_unit_quntity").val("");
        $("#txt_unit_rate").val("100");
        $("#txt_unit_content").val("");
    }

    function del_unit(obj) {
    ...
    <div class="content-tab-ul-wrap">
      <ul>
        ...
        <%if (channel_name == "goods")
          {%>
        <li><a href="javascript:;" onclick="tabs(this);">商品规格</a></li>
        <li><a href="javascript:;" onclick="tabs(this);">商品单位</a></li>
        <li><a href="javascript:;" onclick="tabs(this);">商品别名</a></li>
        <li><a href="javascript:;" onclick="tabs(this);">商品属性</a></li>
        <li><a href="javascript:;" onclick="tabs(this);">商品标签</a></li>
        <%}%>
      </ul>
    </div>
  </div>
...
<div class="tab-content">
  <dl>
    ...
    <dd>
      <div class="rule-single-select">
        <asp:DropDownList id="ddlCategoryId" runat="server" datatype="*" sucmsg=" " AutoPostBack="True"
  ...
  <dl ID="div_good_isput" runat="server" visible="false">
    <dt>上架状态</dt>
    <dd>
      <div class="rule-multi-radio">
        <asp:RadioButtonList ID="isPut" runat="server" RepeatDirection="Horizontal" RepeatLayout="Flow">
  ...
  <dl>
    <dt>会员价格</dt>
    <dd>
      <table border="0" cellspacing="0" cellpadding="0" class="border-table"  cellspacing="2" cellpadding='2' border="1">
  </HeaderTemplate>
        <ItemTemplate>
        <tr>
...
<div id="div_standard" runat="server" visible="false" class="tab-content" style="display:none">
 <dl>
    <dt>规格：</dt>
    <dd id="dd_standard_title" runat="server">
  ...
  <dl>
    <dt></dt>
    <dd id="dd_standard_value" runat="server">
...
<!--/内容-->
<div class="tab-content" id="div_unit" runat="server" style="display:none">

<dl>
    <dt>单位：</dt>
    <dd>
                名称：<input type="text" class="input" id="txt_unit_title" />&nbsp;
                数量：<input type="text" class="input small" id="txt_unit_quntity" />&nbsp;
                比率：<input type="text" class="input small" id="txt_unit_rate"  value="100"/>%&nbsp;
                备注：<input type="text" class="input" id="txt_unit_content" style=" width:250px;" />&nbsp;
                <input type="button" value="添加" class="btn" onclick="add_unit()" />&nbsp;
                <input type="button" value="删除" class="btn yellow" onclick="del_unit()" />
    </dd>
  </dl>

  <dl>
    <dt></dt>
    <dd id="dd_unit_value" runat="server">
      <table>
      <thead>
      <tr>
      ...
      <td style="font-weight:bold;color:#33B5E5;padding-right:10px;">比率</td>
      <td style="font-weight:bold;color:#33B5E5;padding-right:10px;">备注</td>
      <td style="font-weight:bold;color:#33B5E5;padding-right:10px;">操作</td>
      </tr>
      </thead>
      <tbody id="tb_unit_value" runat="server">
...
<div id="div_alias" runat="server" class="tab-content" style="display:none">
 <dl>
    <dt>别名：</dt>
    <dd id="dd_alias" runat="server">
      <asp:CheckBoxList ID="ck_alias" runat="server" RepeatDirection="Horizontal">
...
<div id="div_property" runat="server" class="tab-content" style="display:none">
 
</div>

<div id="div_tag" runat="server" class="tab-content" style="display:none">
 <dl>
    <dt>标签：</dt>
    <dd id="dd_tag" runat="server">
      <asp:CheckBoxList ID="ck_tag" runat="server" RepeatDirection="Horizontal">
