    ...
    public partial class article_list : Web.UI.ManagePage
    {
        ...
        protected void LinkButton1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    bll.UpdateField(id, "is_put=0");
                }
            }
            AddAdminLog(DTEnums.ActionEnum.Edit.ToString(), "下架" + this.channel_name + "频道内容成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
            JscriptMsg("批量下架成功！", Utils.CombUrlTxt("article_list.aspx", "channel_id={0}&category_id={1}&keywords={2}&property={3}",
                this.channel_id.ToString(), this.category_id.ToString(), this.keywords, this.property), "Success");
        }
    }
}