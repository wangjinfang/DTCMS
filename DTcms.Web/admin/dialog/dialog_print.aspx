      ...
      <HeaderTemplate>
        <table width="100%" border="0" cellspacing="0" cellpadding="5" style="font-size:12px; font-family:'微软雅黑'; background:#fff;">
          <tr>
            ...
            <th  align="left" style="background:#ccc;">规格</th>
            <th align="left" style="background:#ccc;">单位</th>
            <td width="12%" align="left" style="background:#ccc;">销售价</td>
            <td width="12%" align="left" style="background:#ccc;">优惠价</td>
            <td width="10%" align="left" style="background:#ccc; display:none;">积分</td>
            <td width="10%" align="left" style="background:#ccc;">数量</td>
            <td width="12%" align="left" style="background:#ccc;display:none;">金额合计</td>
            <td width="12%" align="left" style="background:#ccc;display:none;">积分合计</td>
          </tr>
      </HeaderTemplate>
      <ItemTemplate>
          <tr>
            <td><%#Eval("goods_title")%></td>
            <td><%#new DTcms.BLL.standard_value().get_standrad(Eval("standard").ToString())%></td>
            <td><%#Eval("unit")%></td>
            <td><%#Eval("goods_price")%></td>
            <td><%#Eval("real_price")%></td>
            <td style="display:none;"><%#Convert.ToInt32(Eval("point")) > 0 ? "+" + Eval("point").ToString() : Eval("point").ToString()%></td>
            <td><%#Eval("quantity")%></td>
            <td style="display:none;"><%#Convert.ToDecimal(Eval("real_price"))*Convert.ToInt32(Eval("quantity"))%></td>
            <td style="display:none;"><%#Convert.ToInt32(Eval("point")) * Convert.ToInt32(Eval("quantity"))%></td>
          </tr>
      </ItemTemplate>
      <FooterTemplate>
...
<tr>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="5" style="margin:5px auto; font-size:12px; font-family:'微软雅黑'; background:#fff;">
    ...
    <tr>
      <td valign="top">支付方式：<%=new DTcms.BLL.payment().GetTitle(model.payment_id) %></td>
    ...
    <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" style="border-top:1px solid #000; font-size:12px; font-family:'微软雅黑'; background:#fff;">
      <tr>
        <td align="right">商品金额：￥<%=model.real_amount%> + 配送费：￥<%=model.express_fee%> + 支付手续费：￥<%=model.payment_fee%> -优惠:￥<%=model.payable_amount - model.real_amount%>= 订单总额：<%=model.order_amount%></td>
      </tr>
    </table></td>
  </tr>
</table>
