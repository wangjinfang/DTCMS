...
{
    public partial class address_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            user_id = DTRequest.GetQueryInt("id");

            this.pageSize = GetPageSize(10); //每页数量
            if (!Page.IsPostBack)
            {
                
                RptBind("user_id="+user_id , "add_time desc");
            }
        }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("address_list_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("address_list_page_size", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("address_list.aspx", "user_id={0}",this.user_id.ToString()));
        }
