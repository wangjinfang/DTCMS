...
namespace DTcms.DAL
{
		...
		public partial class property_value
	{
		...
		public bool Exists(decimal id)
		{
			...
			strSql.Append("select count(1) from td_property_value");
			strSql.Append(" where ");
			                                       strSql.Append(" id = @id  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update td_property_value set " + strValue);
...
SqlParameter[] parameters = {
		...
		public bool Delete(decimal id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from td_property_value ");
			strSql.Append(" where id=@id");
						SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Decimal)
			};
		...
		{
			StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM td_property_value ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
		...
		{
			StringBuilder strSql = new StringBuilder();
            strSql.Append("select "+strSelect+" FROM td_property_value ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
