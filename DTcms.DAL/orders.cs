    ...
    public partial class orders
    {
        ...
        public int Add(Model.orders model)
        {
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@is_up",SqlDbType.Int,4),
                    new SqlParameter("@up_status",SqlDbType.Int,4),
            ...
            parameters[29].Value = model.is_up;
            parameters[30].Value = model.up_status;
            ...
            if (model.order_goods != null)
            {
                ...
                foreach (Model.order_goods models in model.order_goods)
                {
                    ...
                    strSql2.Append("order_id,goods_id,goods_title,goods_price,real_price,quantity,point,standard,unit,unit_id,good_no)");
                    ...
                    SqlParameter[] parameters2 = {
						    ...
						    new SqlParameter("@point", SqlDbType.Int,4),
                            new SqlParameter("@standard",SqlDbType.NVarChar,255),    
                            new SqlParameter("@unit",SqlDbType.NVarChar,255),
                            new SqlParameter("@unit_id",SqlDbType.Decimal,9),
                    ...
                    parameters2[7].Value = models.standard;
                    parameters2[8].Value = models.unit;
                    parameters2[9].Value = models.unit_id;
                    parameters2[10].Value = models.good_no;
        ...
        public bool Update(Model.orders model)
        {
            ...
            strSql.Append("is_up=@is_up,");
            strSql.Append("up_status=@up_status");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@order_no", SqlDbType.NVarChar,100),
					new SqlParameter("@trade_no", SqlDbType.NVarChar,100),
        ...
        /// <returns></returns>
        public DataSet Get_Order_Goods_List(int order_id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select goods_title,(select real_name from dt_users where id = (select USER_ID from dt_orders where dt_order_goods.order_id=dt_orders.id)) as real_name,(select img_url from dt_article where dt_article.id=dt_order_goods.goods_id) as img_url,goods_price,quantity,real_price,standard,goods_id from dt_order_goods where order_id=" + order_id);
                return DbHelperSQL.Query(strSql.ToString());
        }
        /// <summary>
        /// 获取商品销量
        ...
        /// <returns></returns>
        public DataSet get_order_good_count(int good_id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select isnull(sum(quantity),0) as quantity from dt_order_goods where goods_id=" + good_id);
            return DbHelperSQL.Query(strSql.ToString());
        }
        #endregion  Method
    }
