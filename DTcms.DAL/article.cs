...
namespace DTcms.DAL
{
    ...
    public partial class article
    {
        ...
        public int Add(Model.article model)
        {
            ...
            strSql.Append("channel_id,category_id,call_index,title,link_url,img_url,seo_title,seo_keywords,seo_description,zhaiyao,content,sort_id,click,status,groupids_view,vote_id,is_top,is_red,is_hot,is_slide,is_sys,is_msg,user_name,add_time,update_time,unit,unit_rate,is_put)");
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@unit_rate", SqlDbType.Decimal,9),
                    new SqlParameter("@is_put",SqlDbType.Int,4),
            ...
            parameters[26].Value = model.unit_rate;
            parameters[27].Value = model.is_put;
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql.Append("unit_rate=@unit_rate,");
                        strSql.Append("is_put=@is_put");
                        strSql.Append(" where id=@id");
                        SqlParameter[] parameters = {
					            new SqlParameter("@channel_id", SqlDbType.Int,4),
					            new SqlParameter("@category_id", SqlDbType.Int,4),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,channel_id,category_id,call_index,title,link_url,img_url,seo_title,seo_keywords,seo_description,zhaiyao,content,sort_id,click,status,groupids_view,vote_id,is_top,is_red,is_hot,is_slide,is_sys,is_msg,user_name,add_time,update_time,unit,unit_rate,is_put");
            strSql.Append(" from " + databaseprefix + "article ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["unit_rate"].ToString() != "")
                {
                    model.unit_rate = decimal.Parse(ds.Tables[0].Rows[0]["unit_rate"].ToString());
                }
                if (ds.Tables[0].Rows[0]["is_put"] != null && ds.Tables[0].Rows[0]["is_put"].ToString() != "")
                {
                    model.is_put = int.Parse(ds.Tables[0].Rows[0]["is_put"].ToString());
                }
                #endregion
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" id,channel_id,category_id,call_index,title,link_url,img_url,seo_title,seo_keywords,seo_description,zhaiyao,content,sort_id,click,status,groupids_view,vote_id,is_top,is_red,is_hot,is_slide,is_sys,is_msg,user_name,add_time,update_time,unit,unit_rate,is_put ");
            strSql.Append(" FROM " + databaseprefix + "article ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
        ...
        /// 获得会员组价格
        /// </summary>
        public decimal GetGroupPrice(int article_id, int group_id)
        {
        ...
        /// <returns></returns>
        public DataSet GetAlbumsList(int top, string strwhere, string strorder)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            if (top > 0)
        ...
        /// <returns></returns>
        public DataSet GetAttributeList(int top, string strwhere, string strorder)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            if (top > 0)
        ...
        /// <returns></returns>
        public DataSet get_order_buy_good(int top,int category_id) {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,title,img_url,sell_price from view_channel_goods where category_id=" + category_id + "  and id in (select top " + top + " goods_id from (select count(0) as sort,goods_id from dt_order_goods group by goods_id) as b order by sort)");
            return DbHelperSQL.Query(strSql.ToString());
        }
    }
}