...
namespace DTcms.DAL
{
		...
		public partial class alias
	{
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from td_alias");
			strSql.Append(" where ");
			                                       strSql.Append(" id = @id  ");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Model.alias model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into td_alias(");			
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update td_alias set " + strValue);
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Model.alias model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update td_alias set ");
...
SqlParameter[] parameters = {
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from td_alias ");
			strSql.Append(" where id=@id");
						SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.alias GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id, category_id, title, add_time  ");			
			strSql.Append("  from td_alias ");
			strSql.Append(" where id=@id");
						SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			Model.alias model=new Model.alias();
		...
		{
			StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM td_alias ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
		...
		{
			StringBuilder strSql = new StringBuilder();
            strSql.Append("select "+strSelect+" FROM td_alias ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
